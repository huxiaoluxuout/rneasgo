...
class BluetoothOTAUpdater {
  ...
  parseIntelHex(hexText, callback) {
    ...
    // 并行计算校验和和生成数据包
    const [totalChecksum, bty16] = this.calculateAndGenerate(
      data.slice(0, dataLength),
    );
    this.totalChecksum = totalChecksum;
    this.byte_16List = bty16;

    let totalChecksumYHex =
      totalChecksum[0].toString(16) + " " + totalChecksum[1].toString(16);
  ...
  calculateAndGenerate(data) {
    ...
    for (let i = 0; i < chunkCount; i++) {
      ...
      const chunk = new Uint8Array(
        buffer,
        i * BluetoothOTAUpdater.CHUNK_SIZE,
        BluetoothOTAUpdater.CHUNK_SIZE,
      );
    ...
    return [[(sum >> 8) & 0xff, sum & 0xff], bty16];
  }
  ...
  buildPacket(index) {
    ...
    let checksum =
      ...
      BluetoothOTAUpdater.DATA_PACKET_LENGTH +
      seq;
  ...
  // 2进制转成16进制
  ab2hex(buffer) {
    return Array.from(new Uint8Array(buffer), (bit) =>
